...
\begin{tikzpicture}[
    ...
    font=\sffamily\small,
    box/.style={draw=bordergray, fill=lightbg, rounded corners=5pt, align=center, minimum width=3.8cm, minimum height=1cm, thick},
    limitbox/.style={draw=accentred!70, fill=accentred!8, rounded corners=5pt, align=center, minimum width=4cm, minimum height=1.2cm, thick},
    goodbox/.style={draw=softgreen!70, fill=softgreen!8, rounded corners=5pt, align=center, minimum width=4cm, minimum height=1.2cm, thick},
    redarrow/.style={-Latex, semithick, accentred!80},
    greenarrow/.style={-Latex, semithick, softgreen!80}
]

    % === EXISTING WORKS (top) ===
    \node (ex_label) [font=\sffamily\small\bfseries, text=accentred] {Existing Research (2023--2026)};

    \node (ex_r1) [box, below=0.5cm of ex_label] {\textbf{Satellite NDVI / Weather Only}\\\footnotesize Sharma (2024), Singh (2025), Cedric (2023)};
    \node (ex_r2) [box, below=0.5cm of ex_r1] {\textbf{State-Level Aggregation / Single Crop}\\\footnotesize Patel \& Kumar (2025), Ramesh (2026)};
    \node (ex_limit) [limitbox, below=0.5cm of ex_r2] {\textbf{Key Limitations}\\\footnotesize No soil NPK $\cdot$ Single crop scope $\cdot$ $R^2 < 0.87$};

    \path [redarrow] (ex_r1) -- (ex_r2);
    \path [redarrow] (ex_r2) -- (ex_limit);

    % === OUR WORK (bottom) ===
    \node (our_label) [font=\sffamily\small\bfseries, text=softgreen, below=1.2cm of ex_limit] {Proposed Framework (This Work)};

    \node (our_r1) [box, below=0.5cm of our_label] {\textbf{Custom Dataset: 27,303 rows}\\\footnotesize Soil (NPK, pH, OC) + Weather + 475 Districts $\cdot$ 19 Crops};
    \node (our_r2) [box, below=0.5cm of our_r1] {\textbf{Soil-Weather Feature Fusion + 8 ML Models}\\\footnotesize LightGBM, XGBoost, Random Forest, DT, SVR, LR};
    \node (our_result) [goodbox, below=0.5cm of our_r2] {\textbf{Results: $R^2 = 0.9312$ $\cdot$ RMSE = 0.5765}\\\footnotesize Interactive Flask Web Dashboard + REST API};

    \path [greenarrow] (our_r1) -- (our_r2);
    \path [greenarrow] (our_r2) -- (our_result);
...
\begin{tikzpicture}[
    node distance=1.0cm,
    font=\sffamily\small,
    stage/.style={draw=#1!70, fill=#1!8, rounded corners=6pt, align=center, minimum width=12cm, minimum height=1.2cm, thick},
    arrow/.style={-Latex, thick, primaryblue!80}
]

    % Vertical pipeline stages
    \node (s1) [stage=primaryblue] {
        \textbf{Stage 1: Data Ingestion} --- Custom CSV Dataset $\cdot$ 27,303 rows $\times$ 21 columns
    };

    \node (s2) [stage=accentpurple, below=of s1] {
        \textbf{Stage 2: Feature Engineering} --- Label Encoding $\cdot$ StandardScaler $\cdot$ Soil (N,P,K,pH,OC) + Weather Fusion
    };

    \node (s3) [stage=accentamber, below=of s2] {
        \textbf{Stage 3: Model Training} --- 8 ML Models $\cdot$ Hyperparameter Tuning $\cdot$ Best: LightGBM ($R^2 = 0.9312$)
    };

    \node (s4) [stage=accentteal, below=of s3] {
        \textbf{Stage 4: Deployment} --- Flask REST API $\cdot$ Chart.js Dashboard $\cdot$ Real-time Inference
    };
